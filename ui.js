...
function onE(e){console.log(e);}
	...
	$('#knoble').toggleClass('knob-off');
		...
		$('#advanced_report').click(function(){   
			var newURL = browser.extension.getURL("popup.html");
			browser.tabs.create({ url: newURL }).then(function(tab){
			console.log(tab);
				browser.runtime.sendMessage({cmd:"id",data:tab.id});

		browser.tabs.executeScript(tab.id,{file:"/popup.js"}).then(function(d){
